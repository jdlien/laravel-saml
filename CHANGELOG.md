...
Initial release as `jdlien/laravel-saml`. Forked from [`overtrue/laravel-saml`](https://github.com/overtrue/laravel-saml) at `c600f90` (the L12 release). Modernized, hardened, and re-published under a new vendor namespace.
...
- Compat shim at `src/compat.php` (auto-loaded via composer `files`) registers `class_alias` for every `Overtrue\LaravelSaml\…` symbol so existing imports keep working during migration. Planned for removal in v2.0.
